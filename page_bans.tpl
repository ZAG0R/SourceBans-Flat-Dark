...
<table width="90%" align="center" border="0" style="border-collapse:collapse;" id="group.details" cellpadding="3">
  ...
  <tr>
	<td colspan='3'>
		<span style='font-size:6pt;color:grey;'>son düzenleyen{$com.edittime} by {$com.editname}</span>
	</td>
  </tr>
  {/if}
  {/foreach}
...
{else}
<h3 align="left">Toplam Ban Sayısı:  <font color="#2C3E50" weight="bold"><u>{$total_bans}</u></font></h3>
<br />
{php} require (TEMPLATES_PATH . "/admin.bans.search.php");{/php}
<br />
<div id="banlist-nav"><a href="index.php?p=banlist&hideinactive={if $hidetext == 'Hide'}true{else}false{/if}{$searchlink|htmlspecialchars}" title="{$hidetext} inactive">{$hidetext} inactive</a> | <i>Toplam Ban: {$total_bans}</i></div>
<div id="banlist">
	<table width="100%" cellspacing="0" cellpadding="0" align="center" class="listtable">
		<tr>
			...
			{/if}
			<td width="12%" height="16" class="listtable_top" align="center"><b>MOD/ÜLKE</b></td>
			<td width="14%" height="16" class="listtable_top" align="center"><b>TARİH</b></td>
			<td height="16" class="listtable_top"><b>Player</b></td>
			{if !$hideadminname}
			<td width="20%" height="16" class="listtable_top"><b>ADMIN</b></td>
			{/if}
			<td width="10%" height="16" class="listtable_top" align="center"><b>SÜRE</b></td>  
		</tr>
        ...
        <td height="16" class="listtable_1">
          ...
          {if empty($ban.player)}
            <i><font color="#677882">Kullanıcı Adı Girilmemiş</font></i>
          {else}
        ...
        {else}
            <i><font color="#677882">Admin Gizlendi</font></i>
        {/if}
        </td>
		{/if}
        ...
        <td colspan="7" align="center">
          <div class="opener">
              ...
              <tr align="left">
                <td width="30%" height="16" class="listtable_1">Oyuncu</td>
                <td height="16" class="listtable_1">
                  {if empty($ban.player)}
                    <i><font color="#677882">yok</font></i>
                  {else}
                    {$ban.player|escape:'html'|stripslashes}
                  {/if}
                </td>
              ...
              {if !$hideplayerips}
              <tr align="left">
                <td width="20%" height="16" class="listtable_1">IP adresi</td>
                <td height="16" class="listtable_1">
                  {if $ban.ip == "none"}
                    <i><font color="#677882">yok</font></i>
                  {else}
              ...
              {/if}
              <tr align="left">
								<td width="20%" height="16" class="listtable_1">Başlangıç Tarihi</td>
								<td height="16" class="listtable_1">{$ban.ban_date}</td>
					        </tr>
					        <tr align="left">
					            <td width="20%" height="16" class="listtable_1">Süresi</td>
					            <td height="16" class="listtable_1">{$ban.banlength}</td>
					        </tr>
							{if $ban.unbanned}
							<tr align="left">
					            <td width="20%" height="16" class="listtable_1">Banın kaldırılma sebebi</td>
					            <td height="16" class="listtable_1">
								{if $ban.ureason == ""}
									<i><font color="#677882">yok</font></i>
								{else}
							 ...
							 <tr align="left">
					            <td width="20%" height="16" class="listtable_1">Banı Kaldıran Admin</td>
					            <td height="16" class="listtable_1">
							...
							{/if}
					        <tr align="left">
					            <td width="20%" height="16" class="listtable_1">Bitiş Tarihi</td>
					            <td height="16" class="listtable_1">
					            	{if $ban.expires == "never"}
		     							<i><font color="#677882">uygulanamaz.</font></i>
		     						{else}
		     							{$ban.expires}
		     						{/if}
		     					</td>
							...
							<tr align="left">
								<td width="20%" height="16" class="listtable_1">Ban Sebebi</td>
								<td height="16" class="listtable_1">{$ban.reason|escape:'html'}</td>
							</tr>
							{if !$hideadminname}
							<tr align="left">
								<td width="20%" height="16" class="listtable_1">Banlayan Admin</td>
								<td height="16" class="listtable_1">
									{if !empty($ban.admin)}
										{$ban.admin|escape:'html'}
									{else}
							...
							{/if}
							<tr align="left">
								<td width="20%" height="16" class="listtable_1">Banlandığı Sunucu</td>
							...
							<tr align="left">
								<td width="20%" height="16" class="listtable_1">Toplam Ban</td>
							...
							<tr align="left">
								...
								<td height="16" class="listtable_1">
								{if $ban.banlog == ""}
									<i><font color="#677882">asla</font></i>
								{else}
									{$ban.banlog}
								{/if}
								</td>
							...
							{if $view_comments}
							<tr align="left">
								<td width="20%" height="16" class="listtable_1">Detaylar</td>
								...
								<table width="100%" border="0">
									{foreach from=$ban.commentdata item=commenta}
									 ...
									 {/if}
									  ...
									  <tr>
										<td colspan='3'>
											<span style="font-size:6pt;color:grey;">son düzenleyen {$commenta.edittime} by {if !empty($commenta.editname)}{$commenta.editname}{else}<i><font color="#677882">Admin deleted</font></i>{/if}</span>
										</td>
									  </tr>
									  {/if}
									  {/foreach}
...
<br />
	...
	&nbsp;&nbsp;L&nbsp;&nbsp;<a href="#" onclick="TickSelectAll();return false;" title="Select All" name="tickswitchlink" id="tickswitchlink">Tümünü Seç</a>&nbsp;&nbsp;|&nbsp;
	<select name="bulk_action" id="bulk_action" onchange="BulkEdit(this,'{$admin_postkey}');">
		<option value="-1">Eylemler</option>
		{if $general_unban}
		<option value="U">Ban Kaldır</option>
		{/if}
		{if $can_delete}
		<option value="D">Ban Sil</option>
		{/if}
	</select>
	<hr>
	{/if}
	{if $can_export }
		<a href="./exportbans.php?type=steam" title="Export Permanent SteamID Bans">Kalıcı SteamID banlarını dışa çıkar.</a>&nbsp;&nbsp;|&nbsp;
		<a href="./exportbans.php?type=ip" title="Export Permanent IP Bans">Kalıcı IP Banlarını dışa çıkar.</a>
	{/if}
</div>
{literal}
